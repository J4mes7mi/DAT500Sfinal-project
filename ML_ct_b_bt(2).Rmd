...
Classification Tree smote
```{r}
trainset_smote$default <- as.factor(trainset_smote$default.payment.next.month)
testset$default <- as.factor(testset$default)
```
...
set.seed(123)
tree_model_smote <- tree(default ~ LIMIT_BAL + SEX + EDUCATION + MARRIAGE + AGE + PAY_0 + PAY_2 + PAY_3 + PAY_4 + PAY_5 + PAY_6, 
                         data = trainset_smote)

plot(tree_model_smote)
text(tree_model_smote, pretty = 0)

tree_pred_smote <- predict(tree_model_smote, testset, type = "class")

conf_matrix_tree_smote <- table(tree_pred_smote, testset$default)
tree_error_smote <- mean(tree_pred_smote != testset$default)
cat("\nSMOTE Tree Confusion Matrix:\n")
print(conf_matrix_tree_smote)
cat("\nSMOTE Classification Tree Test Error Rate:", tree_error_smote, "\n")

tree_accuracy_smote <- sum(diag(conf_matrix_tree_smote)) / sum(conf_matrix_tree_smote)
cat("\nSMOTE Classification Tree Accuracy:", tree_accuracy_smote, "\n")
```
...
trainset_smote <- trainset_smote[, !names(trainset_smote) %in% "default.payment.next.month"]
testset <- testset[, !names(testset) %in% "default.payment.next.month"]

set.seed(123)
bagging_model_smote <- randomForest(default ~ ., 
                                    data = trainset_smote, 
                                    mtry = sqrt(ncol(trainset_smote)),
                                    ntree = 50)

bagging_probs_smote <- predict(bagging_model_smote, newdata = testset, type = "prob")[, 2]
bagging_pred_smote <- ifelse(bagging_probs_smote > 0.5, 1, 0)

conf_matrix_smote <- table(Predicted = bagging_pred_smote, Actual = as.numeric(as.character(testset$default)))
cat("\nSMOTE Bagging Confusion Matrix:\n")
print(conf_matrix_smote)

bagging_accuracy_smote <- sum(diag(conf_matrix_smote)) / sum(conf_matrix_smote)
bagging_error_smote <- mean(bagging_pred_smote != as.numeric(as.character(testset$default)))
cat("\nSMOTE Bagging Accuracy:", bagging_accuracy_smote, "\n")
cat("SMOTE Bagging Test Error Rate:", bagging_error_smote, "\n")
```
...
trainset_smote$default <- as.numeric(as.character(trainset_smote$default))
testset$default <- as.numeric(as.character(testset$default))

set.seed(123)
boosted_model_smote <- gbm(default ~ ., 
                           data = trainset_smote, 
                           distribution = "bernoulli", 
                           n.trees = 5000, 
                           interaction.depth = 4, 
...
boosted_probs_smote <- predict(boosted_model_smote, newdata = testset, n.trees = 5000, type = "response")
boosted_pred_smote <- ifelse(boosted_probs_smote > 0.5, 1, 0)

conf_matrix_boosted_smote <- table(Predicted = boosted_pred_smote, Actual = testset$default)
cat("\nSMOTE Boosted Tree Confusion Matrix:\n")
print(conf_matrix_boosted_smote)

boosted_accuracy_smote <- sum(diag(conf_matrix_boosted_smote)) / sum(conf_matrix_boosted_smote)
boosted_error_smote <- mean(boosted_pred_smote != testset$default)
cat("\nSMOTE Boosted Tree Accuracy:", boosted_accuracy_smote, "\n")
cat("SMOTE Boosted Tree Test Error Rate:", boosted_error_smote, "\n")
```


Comparing Model Performance
```{r}
cat("\nSummary of SMOTE Model Results:\n")
cat("SMOTE Classification Tree Error Rate:", tree_error_smote, "\n")
cat("SMOTE Bagging Error Rate:", bagging_error_smote, "\n")
cat("SMOTE Boosted Tree Error Rate:", boosted_error_smote, "\n")
```
